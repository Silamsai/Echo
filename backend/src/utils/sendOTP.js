...
// Build transporter lazily with env vars
let transporter;
...
export const sendOTPEmail = async (email, otp, username, env) => {
    const transport = getTransporter(env);
    const fromAddress = env.EMAIL_FROM || `"ECHO" <${env.EMAIL_USER}>`;
    const physicalAddress = env.EMAIL_PHYSICAL_ADDRESS || 'ECHO Chat Inc., 123 Tech Loop, San Francisco, CA 94107';
...
export const sendResetEmail = async (email, code, username, env) => {
    const transport = getTransporter(env);
    const fromAddress = env.EMAIL_FROM || `"ECHO Support" <${env.EMAIL_USER}>`;
    const physicalAddress = env.EMAIL_PHYSICAL_ADDRESS || 'ECHO Chat Inc., 123 Tech Loop, San Francisco, CA 94107';
